...
int main() {
	...
	float stdv = 0,mv = 0,kr = 0;	
	for(int i = 0; i < SIZE; i++) {
		input[i] = (float)rand()/(float)(RAND_MAX) * SIZE;mv += input[i];
		//input[i] = (int)rand()%SIZE;
		//input[i] = SIZE - i;
	}
	mv /= SIZE; for(int j=0;j<SIZE;j++){stdv += pow(input[j]-mv, 2);}stdv /= SIZE; printf("Act std = %f\n", sqrt(stdv));
	for(int j=0;j<SIZE;j++){kr += pow(input[j]-mv, 4);}kr /= SIZE; kr /= pow(stdv, 4); printf("Act kr = %f\n", kr);
	...
	cudaMalloc((void**)&_mean, sizeof(float));
