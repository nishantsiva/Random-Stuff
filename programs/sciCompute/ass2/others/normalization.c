...
void normalize(uint8* imageArr, int size){
	...
	for (iter = 0; iter < size-2; iter += 3) {
		imageArr[iter] 		= fabs(imageArr[iter] - meanR) / stdR * meanR;
		imageArr[iter + 1] 	= fabs(imageArr[iter + 1] - meanG) / stdG * meanG;
		imageArr[iter + 2] 	= fabs(imageArr[iter + 2] - meanB) / stdB * meanB;
