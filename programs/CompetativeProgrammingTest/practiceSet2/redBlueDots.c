...
int findMinBlue (double key, int low, int high) {
	int mid = (high + low) / 2;
	if(bluePoints[high].x < key) return -1;
	if(bluePoints[mid].x >= key && low==high) {
		return mid;
	}
...
int findMaxBlue (double key, int low, int high) {
	int mid = MIN((high + low) / 2 + 1, high);
	if(bluePoints[low].x > key) return -1;
	if(bluePoints[mid].x <= key && low==high) {
		return mid;
	}
	if (bluePoints[mid].x <= key) {
		return findMaxBlue(key, mid, high);
	}
	else {
		return findMaxBlue(key, low, mid-1);
	}
}
...
int isEmptyTriag (int p1, int p2, int p3) {
	...
	bl = findMinBlue(MIN(MIN(redPoints[p1].x, redPoints[p2].x), redPoints[p3].x), 0, M-1);
	bh = findMaxBlue(MAX(MAX(redPoints[p1].x, redPoints[p2].x), redPoints[p3].x), 0, M-1);
